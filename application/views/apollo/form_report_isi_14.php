                    ...
                    <table class="table dt-responsive" id="table-res">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    <th scope="row"><?= $no; ?></th>
                                    <td><?= $key->include_text; ?></td>
                                    <td><?= $key->jenis_nasabah; ?></td>
                                    <td><?= $key->sandi; ?></td>
                                    <td><?= $key->nominal; ?></td>
                                    <td><?= $key->jumlah_nasabah_rekening; ?></td>
                                    <td><?= date('d-m-Y', strtotime($key->createdAt)); ?></td>
                                    <td>
                                        <a href="<?= base_url('apollo/form_report_edit_14/' . $key->id); ?>" class="btn rounded-pill btn-primary"><i class='bx bx-edit-alt'></i>
                                        </a>
                                    </td>
                                </tr>
                            <?php };
...
<script>
    $(document).ready(function() {
        $('#table-res').DataTable({
            ...
            buttons: [{
                    extend: 'copyHtml5',
                    messageTop: 'APOLO - Form 00.14 - Data Jenis Nasabah Dan Produk Simpanan di BPR',
                    exportOptions: {
                        columns: [0, 1, 2, 3, 4, 5]
                    }
                },
