...
<script>
    function save_backup() {
        ...
        if (start_date != "" && end_date != "") {
            if (start_date > end_date) {
                ...
                var url = "<?= base_url('sipina/ajax_backup'); ?>";
    ...
    $("#form_report_res").change(function() {
        ...
        if (value !== "") {
            $.ajax({
                type: "POST",
                url: '<?php echo base_url(); ?>sipina/ambil_data_backup',
                cache: false,
                data: {
                    id: value
                },
    ...
    function save_restore() {
        ...
        if (start_date != "" && end_date != "") {
            if (start_date > end_date) {
                ...
                var url = "<?= base_url('sipina/ajax_restore'); ?>";
                // ajax adding data to database
            ...
            if (start_date > end_date) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var url = "<?= base_url('sipina/ajax_backup_all'); ?>";
                        // ajax adding data to database
        ...
        if (start_date != "" && end_date != "") {
            if (start_date > end_date) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var url = "<?= base_url('sipina/ajax_restore_all'); ?>";
                        // ajax adding data to database
    ...
    function view_history_bu(id) {
        $.ajax({
            url: "<?php echo site_url('sipina/view_history_backup/') ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                if (data.status) {
    ...
    function get_detail_bk(id) {
        $.ajax({
            url: "<?php echo site_url('sipina/detail_history_backup/') ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                if (data.status) {
    ...
    function view_history_res(id) {
        $.ajax({
            url: "<?php echo site_url('sipina/view_history_restore/') ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                if (data.status) {
                    ...
                    $.each(data.isi, function(i, v) {
                        $('#list_restore').append(`
                        <a href="<?= base_url('sipina/detail_history_restore/'); ?>` + v.id + `" class="list-group-item list-group-item-action" target="_blank">Nama : ` + v.nm_res + ` Range : ` + v.range + `</a>
                        `)
